...
echo -e '\n' >> ~/.profile
source ~/.profile
sudo apt install bc bison build-essential curl flex g++-multilib gcc-multilib git gnupg gperf libxml2 lib32z1-dev liblz4-tool libncurses5-dev libsdl1.2-dev libwxgtk3.0-gtk3-dev imagemagick git lunzip lzop schedtool squashfs-tools xsltproc xattr zip zlib1g-dev openjdk-8-jdk python3 python-is-python3 perl xmlstarlet virtualenv xz-utils rr jq simg2img libncurses5 pngcrush lib32ncurses5-dev git-lfs libxml2 openjdk-11-jdk-headless -y
mkdir -p ~/bin
